...
(function ($) {
    let calcScrollWidth = () => {
        let html = $('<div style="width:100px;height:100px;overflow:scroll;visibility: hidden;"><div style="height:200px;"></div>');
        $body.append(html);
        let w = (html[0].offsetWidth - html[0].clientWidth);
        $(html).remove();
        return (w);
    }
    let setVH = () => {
    ...
    THEME.shopify = {
        init() {
            if ($('#shopify-section-blog-sidebar').length) this.shopifyBlog();
            this.setLanguage();
            this.scrollNextSection();
            $document.on('click', '.eg-cc-dismiss', function () {
                setTimeout(function () {
                    THEME.setfullheight ? THEME.setfullheight.reinit() : false;
                }, 1000)
            })
        },
        collectionLoaderCheck() {
            if ($('.js-circle-loader-ajax').offset().top < $window.scrollTop() || $window.scrollTop() + window.innerHeight == $document.height()) {
        ...
        shopifyBlog() {
            $('.aside-block').each(function () {
                $(this).find('h2:only-child').parent().remove()
            })
            if ($('.post-prws-grid').length) {
                $('#shopify-section-blog-sidebar').parent().addClass('on-grid');
            } else {
                $('#shopify-section-blog-sidebar').parent().removeClass('on-grid');
            }
            if (!$('#shopify-section-blog-sidebar').is(':visible')) {
                $('#shopify-section-blog-sidebar').parent().addClass('off-sidebar');
            } else {
                $('#shopify-section-blog-sidebar').parent().removeClass('off-sidebar');
            }
        },
        productPageReinit() {
            THEME.product.productPageGallery();
            THEME.productPage.init();
            if ($('.js-countdown').length) THEME.initialization.countdown('.js-countdown');
        },
        frozenButon($button, state) {
            if (state) {
        ...
        scrollNextSection() {
            ...
            $('.js-scroll-to-next-section').on('click', function (e) {
                if ($(this).attr('href') == '#') {
                    e.preventDefault();
                    $(this).blur();
                    let $next = $(this).closest('.shopify-section').next('.shopify-section');
    ...
    THEME.initialization = {
        init() {
            ...
            //if ($('.modal--countdown').length) this.modalCountDown('.modal--countdown');
            ...
            //if ($('.prd-block .prd-has-loader').length) this.imageLoadedProductPage('.prd-block .prd-has-loader');
            //if ($('.prd-has-loader').length) this.imageLoaded('.prd.prd-has-loader, .prd-hor.prd-has-loader, .prd-promo.prd-has-loader, .prd-simple.prd-has-loader');
        ...
        brandsColorHover() {
            let $grid = $('.js-color-hover-brand-grid');
        ...
        popoverHtml(popover) {
            ...
            $(popover).each(function () {
                let parent = $(this).closest('.lookbook-bnr');
                $(this).popover({
                    html: true,
        ...
        videoContent(video) {
            let $videoStopBtn = $('.js-video-stop'),
                $videoPlayBtn = $('.js-video-play');
            $videoPlayBtn.on('click', function () {
                let $video = $(this).closest('.js-video-wrap').find(video);
                $video.get(0).play();
                ...
                let MainSlider = {
                    ...
                    _animate() {
                        ...
                        $slider.on('beforeChange', function (event, slick, currentSlide, nextSlide) {
                            slick = $(slick.$slider);
                            let $nextSlide = $slider.find('.bnslider-slide[data-slick-index="' + nextSlide + '"]'),
                    ...
                    _sizeBackgroundVideos() {
                        function sizeBackgroundVideo($slide) {
                            if ($slide.hasClass('slick-cloned')) {
                                return;
                            }
                            let $player = $slide.find('iframe'),
                    ...
                    _setHeight() {
                        ...
                        function keepScale(slider) {
                            let $bnsliderNoScale = slider;
                            let wW = $(window).width();
                            let mobileBreikpoint = 575;
                            if ($bnsliderNoScale.hasClass("keep-scale") && !$bnsliderNoScale.hasClass("bnslider--fullheight")) {
                                $bnsliderNoScale.css({
                                    'height': '',
                                    'min-height': ''
        ...
        animations() {
            if ($('#morphing').length) {
                let morphing = anime({
                    targets: '#morphing .p',
                    d: [
                        {value: 'M102.415 0.213527C130.863 2.47687 147.861 29.1238 165.824 51.3646C184.061 73.9452 208.291 96.5524 203.349 125.186C198.223 154.882 170.474 174.504 142.619 185.81C117.091 196.172 89.4629 192.662 64.2006 181.665C37.3147 169.962 10.5522 153.227 2.70559 124.905C-5.49916 95.2906 5.82269 64.1385 24.9965 40.1606C44.0991 16.2717 71.9772 -2.20802 102.415 0.213527Z'},
                        {value: 'M85.5803 0.0257094C113.027 0.686367 135.69 17.7373 153.924 38.3289C173.847 60.8277 195.211 87.2464 188.86 116.659C182.461 146.295 151.822 161.123 124.576 174.25C97.3846 187.35 66.591 202.297 39.7448 188.498C13.3189 174.916 8.32449 141.215 3.2131 111.87C-1.45393 85.0772 -3.11404 56.8126 12.9963 34.9367C29.7713 12.1583 57.3593 -0.653592 85.5803 0.0257094Z'},
                        {value: 'M93.5441 2.30824C127.414 -1.02781 167.142 -4.63212 188.625 21.7114C210.22 48.1931 199.088 86.5178 188.761 119.068C179.736 147.517 162.617 171.844 136.426 186.243C108.079 201.828 73.804 212.713 44.915 198.152C16.4428 183.802 6.66731 149.747 1.64848 118.312C-2.87856 89.9563 1.56309 60.9032 19.4066 38.3787C37.3451 15.7342 64.7587 5.14348 93.5441 2.30824Z'}
                    ],
                    easing: 'easeInOutSine',
                    duration: 10000,
                    direction: 'alternate',
            ...
            let $animCollection = $('.collection-item.zoom-out');
            if (!$body.hasClass('touch') && $animCollection.length) {
                $animCollection.on('mouseenter.anime', function () {
                    anime({
        ...
        scrollOnLoad() {
            let $elem = $($(location).attr('href').split('#')[1]);
            if ($elem.length) {
                setTimeout(function () {
                    let speed = $body.height() / 3 > 500 ? $body.height() / 3 : 500;
                    let wHeight = $window.height() < $elem.height() * 2 ? 0 : $window.height() - $elem.height() * 2,
        ...
        simpleFilters(el) {
            if ($(el).length) {
                THEME.simplefilters = {
                    ...
                    _handlers($filterLabel, $galleryItem, $gallery) {
                        let that = this;
        ...
        removeEmpty(selector) {
            $(selector).each(function () {
                let $this = $(this);
                if (!$.trim($this.html()).length) $this.remove();
            })
        },
        removeEmptyParent(selector) {
			$(selector).each(function () {
				let $this = $(this);
        ...
        removeEmptyLinked(target, empty) {
        ...
        backToTop(button) {
            function scrollToTop() {
                ...
                let speed = $window.scrollTop() / 5 > 500 ? $window.scrollTop() / 5 : 500;
                if (isMobile) {
                    speed = speed / 2;
                }
                $('html, body').animate({
        ...
        newsletterModal(modal, checkbox) {
            ...
            function openNewsletterPopup() {
                let pause = $newsletter.attr('data-pause') > 0 ? $newsletter.attr('data-pause') : 2000;
                setTimeout(function () {
        ...
        hideEmptyFilters(columnFilter, filter, mobFilter) {
            if (!$(columnFilter).find(filter).length) {
                $(columnFilter).addClass('d-none');
                $(mobFilter).addClass('d-none');
            }
        },
        hideEmptyFiltersH(columnFilter, filter, mobFilter) {
            if (!$(columnFilter).find(filter).length) {
                $(columnFilter).addClass('d-none');
                $(mobFilter).addClass('d-none');
            }
                ...
                setTimeout(function () {
                    $('.panel-group').each(function () {
                        if ($(this).closest('.holder').data('open-mobile')) {
                            let $first = $(this).find('.panel').first();
            ...
            THEME.flowtype = {
                ...
                init(bnr) {
                    ...
                    $(bnr).each(function () {
                        let $this = $(this);
                        $this.imagesLoaded(function () {
                            let fontratio = Math.round($this.attr('data-fontratio') * 100) / 100;
                            if (fontratio > 0) {
    ...
    THEME.header = {
        ...
        searchAutoFill(parent, link, target) {
                ...
                _handlers(menu) {
                    function setMaxHeight(wHeight, submenu) {
                        ...
                        if (submenu.length) {
                            ...
                            if(submenu.height() > maxH && submenu.hasClass('mmenu-submenu-inside')){
                                submenu.children(':first').css({
                                    //'overflow-y': 'auto'
                                })
                            }
                    ...
                    $('li', $submenu).on('mouseenter', function () {
                        let $this = $(this).addClass('hovered');
                        ...
                        if ($('> .submenu-link-image', $this).length) {
                            let $elm = $('> .submenu-link-image', $this),
                            ...
                            let _$elm = $('.mmenu-submenu', $this).length ? $('.mmenu-submenu', $this) : $('ul:first', $this),
                                _isXvisible = 0,
                                _isYvisible = 0;
                        ...
                        .on('mouseleave', function () {
                            let $elm = $('.mmenu-submenu', this).length ? $('.mmenu-submenu', this) : $('ul:first', this);
                            let $this = $(this).removeClass('to-right').removeClass('hovered');
        ...
        mmobilePush() {
            THEME.mobilemenupush = {
                ...
                _updateActiveMenu(direction) {
                    ...
                    if (direction === 'back') {
                        let curItem = that.defaults.curItem;
                        setTimeout(function () {
        ...
        headerDrop() {
            THEME.headerdrop = {
                ...
                reinitHeight() {
                    ...
                    $this.each(function () {
                        let _ = $(this).find(that.defaults.dropContent);
                        if (!isMobile) {
                            if (!$('.hdr-content-sticky').length) {
                                _.css({
    ...
    THEME.modals = {
                ...
                init(options) {
                    ...
                    $('.js-prd-quickview').fancybox({
                        ...
                        afterShow() {
                            ...
                            that._setScroll();
                            let sections = new debute.Sections();
        ...
        checkOut() {
            $('.js-prd-addtocart[data-fancybox]').fancybox({
                touch: false,
                buttons: ["close"],
                beforeShow() {
                    if ($('.js-prd-carousel-modal-checkout').length) {
                        $('.js-prd-carousel-modal-checkout').fadeOut(0);
                    }
                },
                afterShow() {
                    if ($('.js-prd-carousel-modal-checkout').length) {
                        $('.js-prd-carousel-modal-checkout').fadeIn(300).slick('setPosition');
                    }
                }
            })
            THEME.checkOutModal = {
                defaults: {
                    ...
                    popupAlt: ['.js-popupAddToCompare', '.js-stickyAddToCart', '.js-popupSelectOptions'],
                ...
                setData(data) {
                    ...
                    $(this.defaults.title, $popup).empty().html('<b>' + data.name + ' ' + data.variant_name + '</b>');
                ...
                _setHeigth() {
                    ...
                    for (let i = 0; i < popupAlt.length; ++i) {
            ...
            function createSelect(el) {
                let SelectModal = {
                    defaults: {
                        ...
                        popupAlt: ['.js-popupAddToCompare', '.js-stickyAddToCart', '.js-popupAddToCart'],
                    ...
                    _handlers() {
                        ...
                        // 		setTimeout(function () {
                        // 			that.setData(product_data);
                        // 			that._setFormData(product_data);
                        // 			//that.open();
                        // 		}, 200);
                        // 	} else {
                        // 		that.setData(product_data);
                ...
                if (el == 'selectModal') {
                    ...
                    THEME.selectSticky.init({
                        ...
                        popupAlt: ['.js-popupAddToCompare', '.js-popupSelectOptions', '.js-popupAddToCart']
                    });
                }
            }
        ...
        loaderHorizontal() {
            THEME.loaderHorizontal = {
                defaults: {
                    ...
                    height: 30,
                    popupAlt: ['.js-popupAddToCart', '.js-stickyAddToCart', '.js-popupSelectOptions', '.js-popupAddToCompare']
                },
                init(options) {
                    $.extend(this.defaults, options);
                },
                ...
                _setHeigth() {
                    let $loader = $(this.defaults.loader),
                        popupAlt = $(this.defaults.popupAlt),
    ...
    THEME.product = {
        init() {
            ...
            //this.productWidth('.prd, .prd-hor, .prd-promo');
            //this.countdownWidth('.countdown-box-full-text')
        },
        swatchToSelect() {
            let swatch = '.js-size-list a';
            ...
            $(option).each(function () {
                ...
                $optionbtn.on('click touchstart', function (e) {
                    ...
                    if (!$this.parent('li').is('.active')) {
                        ...
                        $this.parent('li').addClass('active');
                        let text = $this.attr('data-original-title') ? $this.attr('data-original-title') : currentSelect;
        ...
        postAjaxProduct() {
            ...
            THEME.modals.quickView();
            if ($('.js-circle-loader').length) THEME.catalog.setPersentCircle();
            if ($('.has-infinite-scroll .js-circle-loader-ajax').length) THEME.shopify.collectionLoaderCheck();
        },
        colorToggle() {
            THEME.colortoggle = {
                defaults: {
                ...
                _handlers() {
                    ...
                    $document.on('click', that.defaults.toggle, function (e) {
                        ...
                        if ($el.data('image')) {
                            let $prd = $el.closest(that.defaults.product),
                            ...
                            if ($el.data('aspect') && $prd.data('globalAspect') != $el.data('aspect')) {
                                ...
                                let currentAspect = ($image.width() / $image.height()).toFixed(2);
        ...
        productPageGallery(quickview) {
            let ProductPageGallery = {
                defaults: {
                ...
                _galleryBuild() {
                    let that = this,
                        ...
                        _galleryObj = [];
                    $carousel.find('.media-image img').each(function () {
                        let $this = $(this),
                ...
                _zoomLinkEvent() {
                    ...
                    if ($zoomLink.closest('.prd-block').find('.product-previews-carousel').length) {
                        $zoomLink.on('click', function (e) {
                            e.preventDefault();
                            let activeIndex = that._getActiveIndex($carousel),
                ...
                _addZoom($slider, $slide) {
                    let that = this;
                    //$slide.find('img').removeClass('nohover');
                ...
                _handlers() {
                    ...
                    if ($carousel.length) {
                        ...
                        $carousel.on('afterChange', function (event, slick, currentSlide) {
                            ...
                            if (slick.closest(that.defaults.doubleSelector).length) {
                                let previewsRow = currentSlide < 2 ? 0 : Math.floor(currentSlide / 2),
                        ...
                        let $this = $mainMedia.find('img.elzoom');
                        if ($this.data('image-width') > $this.width()) {
                            $this.ezPlus(that.defaults.zoomOptions);
                        }
                    } else if (!isMobile) {
                    ...
                    if ($previews.length) {
                        ...
                        if (that.defaults.quickView) {
                            let vertical = $('.modal-quickview--classic').length ? false : true;
                            $previews.slick({
                                slidesToShow: 4,
                                slidesToScroll: 1,
                                dots: false,
                        ...
                        $previews.find('a').on('click', function (e) {
                            ...
                            if ($this.hasClass('active')) {
                                ...
                                if ($this.closest(that.defaults.doubleSelector).length) {
                                    let slidenumber = $this.closest('.slick-slide').data('slick-index'),
                ...
                switchImage(id) {
                    ...
                    if ($previews.length) {
                        ...
                        // else if ($('.prd-block_images-grid').length && !isMobile) {
                        // 	$slide = $('.prd-block_images-grid').find('[data-image-id=' + id + ']');
                        // 	if ($slide.length) {
                        // 		let h = $('.hdr-content-sticky').length ? $('.hdr-content-sticky').height() : 0;
                        // 		$('html,body').animate({
                        // 			scrollTop: $slide.offset().top - h
                        // 		}, 500);
                        // 	}
                    // }
                    else {
        ...
        actionPrd() {
            THEME.productAction = {
                ...
                clearAllAnimation() {
                    ...
                    let h = $empty[0].scrollHeight;
                    let showEmpty = anime.timeline({
                        easing: 'linear'
                    });
                    showEmpty.add({
                        targets: $prdWrap[0],
    ...
    THEME.productPage = {
        init() {
            ...
            // shopify
            if ($('#shopify-product-reviews').length) this.scrollToReview('.prd-block_reviews', '#shopify-product-reviews');
        },
        countdownCircle() {

            if ($('[data-date]').length && $('[data-locale]') != 'en') {
                let dateArray = $('[data-date]').html().split(',');
                let day = dateArray[0];
                switch (day) {
                    case 'Mon':
                        day = js_helper.date['monday']
                        break;
                    case 'Tue':
                        day = js_helper.date['tuesday']
                        break;
                    case 'Wed':
                        day = js_helper.date['wednesday']
                        break;
                    case 'Thu':
                        day = js_helper.date['thursday']
                        break;
                    case 'Fri':
                        day = js_helper.date['friday']
                        break;
                    case 'Sat':
                        day = js_helper.date['saturday']
                        break;
                    default:
                        day = js_helper.date['sunday']
                }
                dateArray[0] = day;
                ...
                switch (month) {
                    case 'Jan':
                        month = js_helper.date['january']
                        break;
                    case 'Feb':
                        month = js_helper.date['february']
                        break;
                    case 'Mar':
                        month = js_helper.date['marth']
                        break;
                    case 'Apr':
                        month = js_helper.date['april']
                        break;
                    case 'May':
                        month = js_helper.date['may']
                        break;
                    case 'Jun':
                        month = js_helper.date['june']
                        break;
                    case 'Jul':
                        month = js_helper.date['july']
                        break;
                    case 'Aug':
                        month = js_helper.date['august']
                        break;
                    case 'Sep':
                        month = js_helper.date['september']
                        break;
                    case 'Sept':
                        month = js_helper.date['september']
                        break;
                    case 'Oct':
                        month = js_helper.date['october']
                        break;
                    case 'Nov':
                        month = js_helper.date['november']
                        break;
                    default:
                        month = js_helper.date['december']
                }
                dateArray[0] = day;
                dateArray[1] = ' ' + month + ' ' + monthArray[2];
                $('[data-date]').html(dateArray.toString())
            }

            let $countdownCircleTimer = $('.countdownCircleTimer'),
            ...
            let dataTime = $countdownCircleTimer.data('time').split(',');
            let time1 = dataTime[0],
                time2 = dataTime[1],
                time3 = dataTime[2];
            let dt = new Date();
            let date = dt.getFullYear() + "/" + (dt.getMonth() + 1) + "/" + dt.getDate();
            let time = dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds();
            let array = [new Date(date + " " + time1), new Date(date + " " + time2), new Date(date + " " + time3)];
            ...
            let closesTime = closestTime(array, dt),
        ...
        productSold() {
            let $count = $('.js-prd-last-sold-count'),
        ...
        progressBar() {
            THEME.progressbar = {
                ...
                _stockDown(downinterval, stockleft, stockmin) {
                    let that = this,
                        $progress = $(that.data.progress);
                    let _stockleft = stockleft,
                        _stockmin = stockmin;
                ...
                _startCount(downinterval, stockleft, stockmin) {
                    let that = this;
        ...
        stickyAddToCart() {
            THEME.stickyaddtocart = {
                data: {
                    ...
                    popupAlt: ['.js-popupAddToCompare', '.js-popupAddToCart', '.js-popupSelectOptions']
                },
                init(options) {
                    $.extend(this.data, options);
                    let that = this,
                ...
                reinit() {
                    let $stickyAddToCart = $(this.data.stickyAddToCart);
                    if (!$stickyAddToCart.hasClass('closed')) {
                        this._setHeigth($(this.data.stickyAddToCart));
                    }
                },
                _setHeigth($stickyAddToCart) {
                    let popupAlt = this.data.popupAlt,
                ...
                _scrollEvents($stickyAddToCart, $button) {
                    let st = $window.scrollTop();
                    if (st < $button.offset().top) {
                        ...
                        if ($stickyAddToCart.hasClass('closed')) {
                            let h = $stickyAddToCart.find('.container').outerHeight();
                            $stickyAddToCart.removeClass('closed');
        ...
        shiftTitle() {
            if ($('.prd-block-under').length && $('.product-previews-wrapper').length) {
                let shift = $('.product-previews-wrapper').outerWidth() + 15;
        ...
        productTitleReposition() {
            THEME.prdrepos = {
                ...
                _reposBlock(isMobile) {
                    let $prdInfoDesktop = $(this.default.desktop),
        ...
        scrollToReview(link, reviewID) {
            $document.on('click', link, function (e) {
                let $panReview = $(reviewID),
                    tabNavs = '.nav-tabs',
                    tabPane = '.tab-pane',
                    tabPaneM = '.panel',
                    header = '.hdr';
                if ($panReview.length) {
                    if ($panReview.closest(tabPaneM).length) {
                        let $reviewTab = $panReview.closest(tabPaneM).find('.panel-title > a');
                        ...
                        let tabReviewID = $panReview.closest(tabPane).attr('id'),
    ...
    THEME.productFeatures = {
        ...
        paymentNotification(options) {
            THEME.paymentnotification = {
                defaults: {
                    fromArray: 'Aberdeen,Bakersfield,Birmingham,Cambridge,Youngstown',
                    productJson: '[{"productname":"Combat XL", "productlink":"product.html","productimage":"images/products/product-02.jpg"},{"productname":"Green man", "productlink":"product.html","productimage":"images/products/product-03.jpg"},{"productname":"Black Diesel", "productlink":"product.html","productimage":"images/products/product-04.jpg"}]',
                    productArray: '',
                    maxAgoMin: 10,
                    minAgoMin: 1,
                ...
                init(options) {
                    let that = this;
                    if ($.parseJSON(that.defaults.productJson).length == 0) return false;
                    $.extend(that.defaults, options);
                    let $paymentNotification = $(that.defaults.paymentNotification);
                    that.defaults.fromArray = that.defaults.fromArray.split(",");
                    that.defaults.productArray = $.parseJSON(that.defaults.productJson);
    ...
    THEME.catalog = {
        ...
        ajaxShowMore(link) {
            ...
            $link.on('click', function (e) {
                let target = $(this).attr('data-load');
        ...
        sortOptions(obj) {
            $(obj).each(function () {
                ...
                let sortedArray = intersect(sortData, unsortedArray);
        ...
        mobileFilter(el) {
            if ($(el).length) {
                THEME.mobileFilter = {
                    ...
                    close() {
                        let $mobileFilter = $(this.options.mobileFilter);
                        if (isMobile) bodyScrollLock.enableBodyScroll();
                    ...
                    _handlers() {
                        ...
                        $document.on('click', toggleButton, function (e) {
                            ...
                            if ($mobileFilter.hasClass('filter-col--opened')) {
                                let h = $mobileFilter.find('.filter-row-content').outerHeight();
                                $mobileFilter.css({'height': h + 'px'})
                                $mobileFilter.removeClass('filter-col--opened');
                                anime({
        ...
        viewMode(viewmode) {
            ...
            THEME.viewMode = {
                ...
                reinit() {
                    if ($products.hasClass(horizontalClass) && w < maxMD) {
                        $grid.trigger('click')
                    }
                },
                _handlers() {
    ...
    THEME.forms = {
        ...
        checkoutTabs() {
            $('.step-next').on('click', function () {
                let nextId = $(this).closest('.tab-pane').next().attr('id');
        ...
        checkoutAccordion() {
            $('.step-next-accordion').on('click', function () {
                let $nextPanel = $(this).closest('.panel').next('.panel');
                if ($nextPanel) $nextPanel.find('.panel-title > a').trigger('click');
                return false;
            })
        },
        showRecoverPasswordForm() {
            let $link = $('.js-toggle-forms'),
                $form1 = $('#loginForm'),
    ...
    THEME.sections = {
        init() {
            ...
            //if($('.js-post-prw-adaptive').length) this.postWidth('.js-post-prw-adaptive');
        },
        postWidth(post) {
            $(post).each(function () {
                let $this = $(this);
        ...
        arrowCenter(carousel, arrow, image, delay) {
        ...
        creativeSlider(el) {
            ...
            $slider.on('init', function (slick) {
                ...
                //
                // if ($currentSlide.find('.lazyload').length) {
                // 	$currentSlide.find('.col:visible').last().find('.prd-img-first').on('lazyloaded', function () {
                // 		$currentSlide.find('.col').each(function (i) {
                // 			doAnimations($currentSlide.find('.col'), 'fadeInRight', 200);
                // 			doAnimations($currentSlide.find('.creative-product-carousel-text'), 'fadeInRightBig', 0);
                // 		})
                // 	});
                // } else {
                // 	doAnimations($currentSlide.find('.col'), 'fadeInRight', 1000);
                // 	doAnimations($currentSlide.find('.creative-product-carousel-text'), 'fadeInRightBig', 800);
                // }
            });
            $slider.on('afterChange', function (e, slick) {
                slick = $(slick.$slider);
            ...
            $slider.each(function () {
                ...
                $slider.next(tabs).find('.js-collection-grid-nav').on('click', function (e) {
                    ...
                    let url = document.location.origin + $this.find('a').attr('href');
                    if ($this.hasClass('disable')) return false;
                    $this.siblings().removeClass('active').end().addClass('active');
        ...
        slickCarousels() {
            ...
            if ($('.js-post-prws-carousel').length) {
                $('.js-post-prws-carousel').each(function () {
                    let $this = $(this);
                    if ($this.hasClass('slick-initialized')) return false;
                    let arrowsplace = $this.parent().find('.carousel-arrows');
                    $this.on('init', function () {
            ...
            if ($('.js-testimonials-carousel').length) {
                $('.js-testimonials-carousel').each(function () {
                    let $this = $(this);
                    if ($this.hasClass('slick-initialized')) return false;
                    let arrowsplace = $this.parent().parent().find('.carousel-arrows');
                    $this.slick({
                        arrows: true,
                        dots: false,
                        slidesToShow: 2,
            ...
            if ($('.js-brand-carousel').length) {
                $('.js-brand-carousel').each(function () {
                    let $this = $(this);
                    if ($this.hasClass('slick-initialized')) return false;
                    let arrowsplace = $this.parent().find('.carousel-arrows').length ? $this.parent().find('.carousel-arrows') : $this,
                        slidesToShow_lg = 7,
                        slidesToScroll_lg = 1,
                        slidesToShow_md = 5,
                        slidesToScroll_md = 1,
                    ...
                    // $document.on('click', '.js-brand-carousel .slick-next', function(){
                    // 	clearTimeout(slickTimer);
                    // 	$this.slick('slickSetOption', 'speed', 500, true);
                    // 	$this.slick('slickSetOption', 'waitForAnimate', false, false);
                    // 	$this.slick('slickSetOption', 'autoplay', false, true);
                    // 	$this.slick('slickNext');
                    // 	slickTimer = setTimeout(function () {
                    // 		$this.slick('slickSetOption', 'speed', 1500, true);
                    // 		$this.slick('slickSetOption', 'autoplay', true, true);
                    // 		$this.slick('slickSetOption', 'waitForAnimate', true, false);
                    // 	}, 500)
                    // });
                    // $document.on('click', '.js-brand-carousel .slick-prev', function(){
                    // 	clearTimeout(slickTimer);
                    // 	$this.slick('slickSetOption', 'speed', 500, true);
                    // 	$this.slick('slickSetOption', 'waitForAnimate', false, false);
                    // 	$this.slick('slickSetOption', 'autoplay', false, true);
            ...
            THEME.prdcarousel = {
                ...
                reinit() {
                    ...
                    $(this.defaults.carousel).each(function () {
                        ...
                        $this.on('init', function () {
                            THEME.product.productWidth('.prd', $this);
                            //THEME.initialization.imageLoaded($('.prd.prd-has-loader', $this), true);
                        })
                        let slidesToShow = 4,
                            slidesToShowLG = 4,
                            slidesToShowMD = 3,
                ...
                THEME.productisotopeSM = {
                        ...
                        if ($filtersList.length) {
                            $gallery.imagesLoaded(function () {
                                $gallery.isotope({
                                    ...
                                    filter() {
                                        let filterResult = that.defaults.currentFilter ? $(this).is(that.defaults.currentFilter) : true;
                                        return filterResult;
                                    }
                                });
                            });
                    ...
                    _filters(obj) {
                        ...
                        $filtersLabel.each(function () {
                            ...
                            let filtered = $this.attr(dataAttr),
        ...
        productIsotope(el) {
            if ($(el).length) {
                THEME.productisotope = {
                    ...
                    init(options) {
                        $.extend(this.defaults, options);
                        let $gallery = $(this.defaults.gallery),
                    ...
                    reinit() {
                        ...
                        if ($filtersList.length) {
                            ...
                            let $gallery = $(this.defaults.gallery);
                            let ltr = $body.is('.rtl') ? false : true;
                            $gallery.isotope({
                                isOriginLeft: ltr
                            });
                            return this;
                    ...
                    _galleryInit(obj) {
                        let $gallery = $(obj.defaults.gallery),
                            $filtersList = $(obj.defaults.filtersList, $gallery.closest('.holder'));
                        if ($filtersList.length) {
                            let ltr = $body.is('.rtl') ? false : true;
                            $gallery.imagesLoaded(function () {
                                $gallery.isotope({
                                    ...
                                    percentPosition: true,
                                    filter() {
                                        let filterResult = obj.defaults.currentFilter ? $(this).is(obj.defaults.currentFilter) : true;
                                        return filterResult;
                                    }
                                });
                            });
            ...
            if ($(el).length) {
                THEME.gallery = {
                    ...
                    _popup(obj) {
                        ...
                        let $popupImage = $(obj.defaults.gallery + ' ' + obj.defaults.popupImage);
            ...
            THEME.carouseltab = {
                ...
                reinit() {
                    ...
                    $(that.defaults.carousel).each(function () {
                        ...
                        let slidesToShow = parseInt($this.attr('data-to-show'), 10);
                        if (w < maxMD) {
                            slidesToShow = 3
                        }
                        $this.slick({
        ...
        instaFeed(el) {
            $(el).each(function () {
                ...
                function startInstagramCarousel(carousel) {
                    let $carousel = $(carousel).find('.instagram_gallery');
                    ...
                    let arrowsplace = $carousel.closest('.holder').find('.carousel-arrows');
                    $carousel.slick({
                        speed: 500,
                        slidesToShow: 7,
                        slidesToScroll: 2,
    ...
    THEME.documentLoad = {
        init() {
            ...
            THEME.modals.init();
            if ($('.has-infinite-scroll .js-circle-loader-ajax').length) {
                THEME.shopify.collectionLoader();
                THEME.shopify.collectionLoaderCheck();
            }
            if (productPage) THEME.productPage.init();
            //THEME.productFeatures.init();
            //THEME.productFeatures.paymentNotification({fromArray: 'Odessa, Kherson', productJson: '[{"productname":"TEST", "productlink":"product.html","productimage":"images/products/product-02.jpg"}]'})
        }
    };
    ...
    $document.on('shopify:section:select shopify:section:load', function () {
        THEME.sections.slickCarousels();
        THEME.shopify.shopifyBlog();
        THEME.flowtype ? THEME.flowtype.reinit('.bnr[data-fontratio]') : false;
        THEME.product.productWidth('.prd, .prd-hor, .prd-promo');
        if (productPage) THEME.shopify.productPageReinit();
        if ($('.js-instagram-feed').length) THEME.sections.instaFeed('.js-instagram-feed');
        if ($('.bnslider').length) THEME.initialization.mainSlider('.bnslider');
        THEME.initialization.hoverColor();
        if ($('.bnslider--fullheight').length) THEME.initialization.setFullHeightSlider('.bnslider--fullheight');
        if ($('.js-creative-product-carousel').length) THEME.sections.creativeSlider('.js-creative-product-carousel');
        if ($('.js-countdown').length) THEME.initialization.countdown('.js-countdown');
    });
})(jQuery)